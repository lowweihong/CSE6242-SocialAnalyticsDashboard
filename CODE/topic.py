...
def load_file(path='./res'):
    ...
    if os.path.isfile(path):
        ...
        if path.endswith('.parquet'):
            ...
            df_list.append(df)
            print(f'imported file: {os.path.basename(path)}')
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path', type=str, default='./res',
                        help='Path to the directory containing input files (default: ./res)')
    args = parser.parse_args()

    combined_df = load_file(args.input_path)
    main(combined_df)
